...
if(strstr($_FILES['qqfile']['name'], '.')){
    $Filename=explode('.', $_FILES['qqfile']['name']);
    $Filename=$_POST['qquuid'].'.'.$Filename[1];
}else
    $Filename =$_POST['qquuid'];

//$Filename=$_POST['qquuid'].'_'.$_FILES['qqfile']['name'];
...
$result = $uploader->handleUpload('uploads',$Filename);
